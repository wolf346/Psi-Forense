...
ITEMS_MMPI2RF = [
    ...
    "286. No le tengo miedo al ratones.",
...
MAPA_TESTS = {
    ...
    "MCMI-III": {"items": ITEMS_MCMIIII, "opciones": None},
...
if st.session_state["perito_autenticado"]:
  ...
  if st.session_state.get("token_finalizado_exito"):
    st.success("¡Evaluación finalizada y guardada correctamente bajo cadena de custodia! Ya puede cerrar esta ventana.")
    if st.button("Aceptar e ir al inicio", use_container_width=True):
      del st.session_state["token_activo"]
      del st.session_state["token_finalizado_exito"]
      st.rerun()

  elif "token_activo" not in st.session_state:
    ...
    if datos_token.get("estado") == "finalizado":
      ...
      # -----------------------------------------------------------------------
        ...
        if st.session_state.get("test_enviado"):
          st.success(
              "¡Escala guardada y registrada con éxito bajo cadena de custodia!"
          )
          st.write(
          ...
          col_btn_otro, col_btn_fin = st.columns(2)
          with col_btn_otro:
            if st.button(
                "📝 Realizar otra escala",
                use_container_width=True,
            ):
              st.session_state["test_enviado"] = False
              st.rerun()
          with col_btn_fin:
            if st.button(
                "🔒 Finalizar toda la evaluación",
                type="primary",
                use_container_width=True,
            ):
              datos_token["estado"] = "finalizado"
              guardar_token_db(token_actual, datos_token)
              st.session_state["token_finalizado_exito"] = True
              st.rerun()
        else:
          ...
          if test_seleccionado == "Listado de Síntomas Breve (LSB-50)":
            ...
            with st.form("form_lsb50"):
              ...
              if st.form_submit_button(
                  "Guardar y Enviar LSB-50", use_container_width=True
              ):
          ...
          # Escala de Sucesos de Vida (Casullo)
            ...
            with st.form("form_casullo"):
              for idx, preg in enumerate(ITEMS_CASULLO, 1):
                ...
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar Escala de Casullo",
                  use_container_width=True,
              ):
          ...
          # B) MMPI-2-RF
            ...
            with st.form("form_mmpi2rf"):
              for idx, preg in enumerate(ITEMS_MMPI2RF, 1):
                ...
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar MMPI-2-RF", use_container_width=True
              ):
          ...
          # MCMI-III
            ...
            st.subheader("MCMI-III (Inventario Clínico Multiaxial de Millon-III)")
            st.info(
                "Responda a cada una de las afirmaciones marcando"
                " **Verdadero** o **Falso** según se aplique a usted."
            )
            respuestas_mcmi = {}
            with st.form("form_mcmi3"):
              for idx, preg in enumerate(ITEMS_MCMIIII, 1):
                respuestas_mcmi[f"p_{idx}"] = st.radio(
                    preg,
                    options=OPCIONES_MCMI,
                    horizontal=True,
                    key=f"mcmi_{idx}",
                )
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar MCMI-III", use_container_width=True
              ):
          ...
          # C) CUIDA
            ...
            with st.form("form_cuida"):
              for idx, preg in enumerate(ITEMS_CUIDA, 1):
                ...
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar CUIDA", use_container_width=True
              ):
          ...
          # D) STAI
            ...
            with st.form("form_stai"):
              for idx, preg in enumerate(ITEMS_STAI, 1):
                ...
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar STAI", use_container_width=True
              ):
          ...
          # E) BDI-II
            ...
            with st.form("form_bdii"):
              for idx, item in enumerate(ITEMS_BDI, 1):
                ...
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar BDI-II", use_container_width=True
              ):
          ...
          # F) PAI
            ...
            with st.form("form_pai"):
              for idx, preg in enumerate(ITEMS_PAI, 1):
                ...
                st.divider()
              if st.form_submit_button(
                  "Guardar y Enviar PAI", use_container_width=True
              ):
