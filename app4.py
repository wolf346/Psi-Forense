...
st.set_page_config(page_title="Evaluaciones Psicologicas Forenses - SIN MMPI", page_icon="⚖️", layout="wide")
...
st.sidebar.title("Sistema Forense Online")
rol = st.sidebar.radio("Como queres ingresar?", ["Soy Perito (Admin)", "Soy Evaluado (con Token)"], index=0)

if rol == "Soy Perito (Admin)":
    st.title("Panel Perito - Control Central (Sin MMPI)")
    ...
    with col_a:
        if st.button("ACTUALIZAR DATOS AHORA", type="primary", use_container_width=True):
            ...
            st.rerun()
    with col_b:
        st.caption(f"Version datos: {st.session_state['data_version']} | {datetime.now(TZ).strftime('%d/%m %H:%M:%S')}")
    with col_c:
        if st.button("Cerrar sesion perito"):
            st.session_state["perito_autenticado"] = False
            st.rerun()
    st.divider()
    datos = cargar_datos_db()
    st.subheader("1 Crear nuevo evaluado")
    with st.form("crear_token"):
        ...
        if st.form_submit_button("Generar Link + Token"):
            ...
            guardar_token_db(nuevo_token, {"estado": "activa", "datos_persona": {"nombre": nombre, "dni": dni, "causa": causa, "obs": obs}, "evaluaciones": {}, "ip_acceso": ip, "user_agent": ua})
            ...
            st.rerun()
    st.divider()
    st.subheader(f"2 Evaluaciones ({len(datos)}) - MMPI quitado")
    if not datos:
        ...
        for tok,info in datos.items():
            ...
            filas.append({"Token":tok, "Nombre":dp.get("nombre","-"), "DNI":dp.get("dni","-"), "Estado":info.get("estado"), "Tests": ", ".join(evals.keys()) if evals else "-", "IP":info.get("ip_acceso")})
        df=pd.DataFrame(filas)
        st.dataframe(df, use_container_width=True, hide_index=True)
        st.markdown("### 3 Descargar resultados al Actualizar")
        ...
        output_excel.seek(0)
        c1,c2 = st.columns(2)
        c1.download_button("DESCARGAR EXCEL COMPLETO", data=output_excel, file_name=f"forense_sin_mmpi_{datetime.now(TZ).strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, type="primary")
        json_data = json.dumps(datos, indent=2, ensure_ascii=False)
        c2.download_button("DESCARGAR JSON", data=json_data, file_name=f"backup_sin_mmpi_{datetime.now(TZ).strftime('%Y%m%d_%H%M')}.json", mime="application/json", use_container_width=True)
        st.divider()
    ...
    st.title("Evaluacion Psicologica - Acceso Evaluado")
    query_params = st.query_params
    token_url = query_params.get("token", None)
    token_input = st.text_input("Ingresa tu TOKEN", value=token_url if token_url else "", placeholder="EVAL-XXXXXX")
    if token_input:
        st.session_state["token_actual"] = token_input.strip().upper()
    token_actual = st.session_state["token_actual"]
    if not token_actual:
        st.info("Pedile a tu perito el token")
        st.stop()
    ...
    if st.session_state["test_enviado"]:
        st.success("Enviado correctamente al perito. Ya podes cerrar.")
        if st.button("Hacer otro test"):
            st.session_state["test_enviado"] = False
            st.rerun()
        st.stop()
    st.success(f"Token valido: {token_actual} | Evaluado: {datos_token['datos_persona'].get('nombre','-')}")
    test_seleccionado = st.selectbox("Selecciona el test asignado", TESTS_DISPONIBLES)
    if test_seleccionado.startswith("LSB-50"):
        ...
        st.info("Responda Verdadero o Falso segun se aplique a usted.")
        respuestas_mcmi={}
